  ...
  module Model
    ...
    macro has_many(klass, *, rel_type, name = "", unique = true)
      ...
      class ::Neo4jModel::QueryProxy(T)
        ...
        def {{@type.id.underscore}}_has_many_{{name}}(assoc_obj_variable_name = :{{name}}, assoc_rel_variable_name = :r)
      ...
      def {{name}}(assoc_obj_variable_name = :{{name}}, assoc_rel_variable_name = :r)
        proxy = query_proxy.{{@type.id.underscore}}_has_many_{{name}}(assoc_obj_variable_name, assoc_rel_variable_name)
