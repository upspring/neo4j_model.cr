          ...
          if (val = hash["{{var}}"]).nil?
            ...
            {% if var.type <= Bool || (var.type.union? && var.type.union_types.includes?(Bool)) %}
              ...
              when String
                ...
                raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Bool (attribute: {{var.id}})"
              end
            {% elsif var.type <= Integer || (var.type.union? && (var.type.union_types.includes?(Int8) || var.type.union_types.includes?(Int16) || var.type.union_types.includes?(Int32) || var.type.union_types.includes?(Int64))) %}
              ...
              when String
                ...
                raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Integer (attribute: {{var.id}})"
              end
            {% elsif var.type <= Float64 || (var.type.union? && var.type.union_types.includes?(Float64)) %}
              case val
              when Number
                self.{{var}} = hash["{{var}}"].as(typeof(@{{var}}))
              when String
                ...
                raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Float64 (attribute: {{var.id}})"
              end
            {% elsif var.type <= Time || (var.type.union? && var.type.union_types.includes?(Time)) %}
              ...
              when String
                if val.blank?
                  ...
                  raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Time (attribute: {{var.id}})"
                end
              else
                raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Time (attribute: {{var.id}})"
              end
            {% elsif var.type <= Array(String) || (var.type.union? && var.type.union_types.includes?(Array(String))) %}
              ...
              when String
                ...
                raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Array(String) (attribute: {{var.id}})"
              end
            {% elsif var.type <= Hash(String, String) || (var.type.union? && var.type.union_types.includes?(Hash(String, String))) %}
              ...
              when String
                ...
                raise "Neo4jModel doesn't know how to convert #{val.class.name} '#{val}' to Hash (attribute: {{var.id}})"
              end
    ...
    def save(*, skip_callbacks = false) : Bool
      ...
      unless skip_callbacks
        unless @@_before_save_callback.call(self)
          Neo4jModel::Log.debug { "before_save callback returned false, aborting" }
          return false
        end
      end
      ...
      unless skip_callbacks
        unless @@_after_save_callback.call(self)
          Neo4jModel::Log.debug { "after_save callback returned false, aborting" }
          return false
        end
      end
