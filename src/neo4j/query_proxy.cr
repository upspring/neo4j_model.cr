module Neo4jModel
  ...
  class QueryProxy(T)
    ...
    # bring in from model object for easier access
    property label : String = ""
    property uuid : String?

    @_objects = Array(T).new
    @_rels : Array(Neo4jModel::Relationship) = Array(Neo4jModel::Relationship).new
    ...
    # for associations
    annotation InstanceProxy; end
    annotation AddProxy; end
    annotation DeleteProxy; end

    property add_proxy : Neo4jModel::QueryProxy(T)?
    property delete_proxy : Neo4jModel::QueryProxy(T)?

    # forward_missing_to instance_proxy
    # def instance_proxy(name)
    #   puts "FIXME: implement instance_proxy(#{name})"

    #   #{#% for method in Neo4jModel::QueryProxy.methods.select { |m| m.annotation(InstanceProxy) } %}
    #   # puts "{#{method}}"
    #   # if annotation(instance_proxy)
    #   #{#% end %}
    # end

    def initialize(@label,
                   @match = "",
                   @ret = "RETURN #{T.name.underscore}") # all other parameters are added by chaining methods
      # puts "#{caller}: new query proxy for #{T.name}; label = #{@label}"
      @match = "MATCH (#{T.name.underscore}:#{@label})" if @match.blank?
      @obj_variable_name = T.name.underscore
    end
    ...
    def chain(proxy : QueryProxy)
      if proxy.match.starts_with?(match)
        # all good
      else
    ...
    def query_as(obj_var : (Symbol | String)) : QueryProxy(T)
      @obj_variable_name = obj_var.to_s
      self
    end

    def query_as(obj_var : (Symbol | String), rel_var : (Symbol | String)) : QueryProxy(T)
      @rel_variable_name = rel_var.to_s
      query_as(obj_var)
    end

    # clone the query, not the results
    def clone_for_chain : QueryProxy(T)
      new_query_proxy = self.class.new(@label, @match, @create_merge, @ret)
    ...
    def order(prop : Symbol, dir : SortDirection = SortDirection::ASC) : QueryProxy(T)
      @order_bys << expand_order_by(prop, dir)
      clone_for_chain
    end

    # ex: .order(:name, :created_by)
    def order(*params) : QueryProxy(T)
      params.each do |prop|
    ...
    def unorder : QueryProxy(T)
      @order_bys.clear
      clone_for_chain
    end

    def reorder(**params) : QueryProxy(T)
      @order_bys.clear
      order(**params)
    end
    ...
    def skip(@skip) : QueryProxy(T)
      clone_for_chain
    end

    def limit(@limit) : QueryProxy(T)
      clone_for_chain
    end

    def distinct : QueryProxy(T)
      @ret_distinct = true
      clone_for_chain
    end

    def reset_query : QueryProxy(T)
      @cypher_query = nil
    ...
    def delete(obj : String | T) : QueryProxy(T)
    ...
    def set_label(label : Symbol | String) : QueryProxy(T)
      @sets << {"#{obj_variable_name}:#{label.to_s}", ParamsHash.new}
      clone_for_chain
    end

    def remove_label(label : Symbol | String) : QueryProxy(T)
    ...
    def execute(skip_build = false, skip_return = false) : QueryProxy(T)
      build_cypher_query(skip_return: skip_return) unless skip_build

      T.with_connection do |conn|
      ...
      if (result = raw_result)
        @_objects = Array(T).new
        @_rels = Array(Neo4jModel::Relationship).new
        @return_values.clear
            ...
            if (fields == [obj_variable_name.to_s, rel_variable_name.to_s]) || # the most common case
              ...
              if (node = data[0]?.try &.as?(Neo4j::Node))
                ...
                if (rel = data[1]?.try &.as?(Neo4j::Relationship))
                  @_rels << T::Relationship.new(rel, clone_for_chain.where(uuid: obj.uuid))
                end
              end
